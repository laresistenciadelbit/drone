//nota de mapeo para el cliente en google keep
...
//MI DRON FUE HECHO CON PRO MICRO 3.3V <- NO LO BRIQUEES!

// 0 .__. 1    <--motors
//   |  |
// 2 .__. 3 

// ***** <- significa atención o falta de código
...
const int motor_pin[4]={9,6,5,10}; //pines de los motores
int speed[4]={0,0,0,0};//vector de velocidades
const int starting_speed=55;//220; //0-255 / es un valor sin medir, luego se estabilizara el solo.
int stable[4]={starting_speed,starting_speed,starting_speed,starting_speed};// 0-255 vector de velocidad estable (se reiniciaran solos los valores)
...
void get_angle()	// 10ms por cada lectura?
{
    ...
    if ((mpuIntStatus & 0x10) || fifoCount == 1024) {
        ...
        fifoCount -= packetSize;
            ...
            //ROLL = ypr[0] * 180/M_PI;	//no necesitamos el angulo de giro para la estabilizacion
            x = ypr[1] * 180/M_PI;
            y = ypr[2] * 180/M_PI;
    }
}
...
void stabilize_xyz(int no_stable_xy=45)
{
	do{
		...
		if(get_command(&p1,&p2))
			if(p1==111)
				HARDSTOP=true;

	}while(!is_stable() && !HARDSTOP);	//mientras no sea estable xyz.
...
void loop()
{
  //[]REINICIAMOS VARIABLES
      ...
      stable[0]=stable[1]=stable[2]=stable[3]=starting_speed;
      x=y=z=x_aux=y_aux=z_aux=0;
      time_started=false;
      HARDSTOP=false;
      p1=0,p2=0;
  //[]END
  ...
  byte byte250div=0,byte250measures=0;  //uno cuenta divisiones de 250ms hasta 1s, y el otro hasta 8veces 250ms
    ...
    if(HARDSTOP)//parada forzosa durante la estabilizacion
    {
      hard_stop();
      return;//salimos
    }
