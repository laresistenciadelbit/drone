//fallo: la estabilización en eje x se calcula cada vez que estabilizamos,
//eso no puede ser así, ya que esa altura la calcula siempre a 42cm del suelo.
//tenemos que aumentarla a 1metro? y el dron almacenará esas velocidades de estabilidad 
//para los motores, cuando el dron llegue a esa altura (80-120cm) si que recalculará la estabilidad z
...
const int lost_height_to_stabilize=75;//75cm perdidos antes de nueva estabilización de altura.
const int ms_to_strict_stabilize=1000; //1segundo antes de estabilización estricta.
...
const int stable_z=100;//altura de estabilización la ponemos a 100cm. (para tener un margen entre 90 y 110cm)
const int Zmargin=20/2; //( 20/2 = 10cm por encima y 10cm por debajo de 1m)
...
void setup()
{
	...
	//{Acc  config BEGIN}
		...
		#ifdef PROMICRO
			attachInterrupt(2, dmpDataReady, RISING);	//2->pin0(rx) for pro micro
		#else 
			attachInterrupt(0, dmpDataReady, RISING);	//0->pin2 for uno(also pro micro but it uses pin2 for sda)
		#endif
		mpuIntStatus = accelgyro.getIntStatus();
		dmpReady = true;
...
void loop()
{
	...
	//stabilize:
		...
		if(time_diff<ms_to_strict_stabilize)	stabilize_xyz();//stabilize dron with {no_stable_xy_limit} 
		else { //If not receive command in 1.5 seconds: stabilize drone with {no_stable_xy_strict}
			stabilize_xyz(no_stable_xy_strict);
			time_started=false;
			time_start = millis();
			for(int i=0;i<3;i++)	stable[i]=speed[i];
			
			second_counter++; //contamos los segundos en los que no se le ha mandado comandos.
		}
		//stabilize_z(); called in stabilize_xyz();
		
		if(second_counter>120) stop();//CONTAR 60 SEGUNDOS Y VOLVER A LA BASE <- de momento baja y se apaga
	}while(p1!=100);
