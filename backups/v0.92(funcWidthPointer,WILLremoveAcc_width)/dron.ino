...
int *drone_width(); //funcion puntero que contendrá HC-SR04(ultrasonidos) o MS5611(presion atm) según cual de los dos se use
...
{
	int medidas=3;
	int width=*drone_width(); //Medida inicial
...
void setup()
{
	for(int i=0; i<4; i++) pinMode(motor_pin[i], OUTPUT);
		...
		drone_width=F_HC-SR04;	//metemos la funcion en una funcion puntero
	#endif
		...
		drone_width=F_MS5611;	//metemos la funcion en una funcion puntero
	#endif
